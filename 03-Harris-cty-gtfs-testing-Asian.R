...
pre_wmd #population weighted mean before = 45.07 departures/day
pre_wmh <- weighted.mean(pre_stop_acs_int$headway, pre_stop_acs_int$pop_est_new, na.rm = FALSE)
pre_wmh #population weighted mean headway before = 31.00 minutes of those within 400 m

post_wmd <- weighted.mean(post_stop_acs_int$departures, post_stop_acs_int$pop_est_new, na.rm = FALSE)
post_wmd #population weighted mean after = 49.48 departures/day
post_wmh <- weighted.mean(post_stop_acs_int$headway, post_stop_acs_int$pop_est_new, na.rm = FALSE)
post_wmh #population weighted mean headway before = 27.93 minutes of those within 400 m
